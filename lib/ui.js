...
export async function initUI() {
  ...
  // 4. Load profile (client read, with server API fallback)
  let profile = null;
  if (user) {
...
export async function initAdminUI() {
  ...
  // Extra safety: verify role via server API if client read didn't give admin
  if (role !== 'admin') {
...
async function loadProfile(user) {
  // Attempt 1: direct client read (respects RLS)
  try {
  ...
  // Attempt 2: server API (service role, bypasses RLS)
...
/**
 * Call this after any balance-changing action (buy, sell, deposit, etc.)
 * to refresh the header chip without a page reload.
 */
...
function setupFloatingFooter(user, currentPath) {
  ...
  // No floating nav on public or admin pages
